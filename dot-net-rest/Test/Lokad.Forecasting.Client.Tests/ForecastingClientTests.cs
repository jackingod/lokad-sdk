...
namespace Lokad.Forecasting.Client.Tests
{
    ...
    public class ForecastingClientTests
    {
        ...
        [Test]
        public void GetQuantiles_IsForwarded()
        {
            string identity = "mockid", datasetName = "mydataset";
            ...
            using (mocks.Record())
            {
                ...
                Expect.Call(api.GetQuantileStatus(identity, datasetName))
                    .Return(new ForecastStatus
                        {
                            ForecastsReady = true
                        });
                ...
                //  180 time-series => 4 requests
                Expect.Call(api.GetQuantiles(identity, datasetName, null))
                    .Return(new QuantileCollection
                        {
                            Quantiles = new[] { new QuantileValue { Name = timeSeries[0].Name } }
                        })
                    .IgnoreArguments();
                Expect.Call(api.GetQuantiles(identity, datasetName, null))
                    .Return(new QuantileCollection
                        {
                            Quantiles = new[] { new QuantileValue { Name = timeSeries[1].Name } }
                        })
                    .IgnoreArguments();
                Expect.Call(api.GetQuantiles(identity, datasetName, null))
                    .Return(new QuantileCollection
                        {
                            Quantiles = new[] { new QuantileValue { Name = timeSeries[2].Name } }
                        })
                    .IgnoreArguments();
                Expect.Call(api.GetQuantiles(identity, datasetName, null))
                    .Return(new QuantileCollection
                        {
                            Quantiles = new[] { new QuantileValue { Name = timeSeries[3].Name } }
                        })
                    .IgnoreArguments();
            }
            ...
            {
                var client = new ForecastingClient(identity, api);
                var forecasts = client.GetQuantiles(datasetName, timeSeries.Select(d => d.Name).ToArray());
                Assert.IsNotNull(forecasts, "#A00");
                Assert.AreEqual(4, forecasts.Length, "#A01");
                Assert.AreEqual(timeSeries[0].Name, forecasts[0].Name, "#A02");
                Assert.AreEqual(timeSeries[1].Name, forecasts[1].Name, "#A03");
